...
func generateRouterStatus() {
	...
	req, err := http.NewRequest("POST", "http://" + os.Getenv("TPIP") + "/data/router.status.json", body)
	if err != nil {
	...
	req.Header.Set("Referer", "http://" + os.Getenv("TPIP") + "/index")
...
func generateClientStatus() {
	...
	body := strings.NewReader(`operation=read`)
	req, err := http.NewRequest("POST", "http://" + os.Getenv("TPIP") + "/data/device.all.json", body)
	req.Header.Set("Accept", "application/json, text/javascript, */*; q=0.01")
	req.Header.Set("Accept-Language", "en-GB,en;q=0.5")
	req.Header.Set("Referer", "http://" + os.Getenv("TPIP") + "/index")
	req.Header.Set("Content-Type", "application/x-www-form-urlencoded; charset=UTF-8")
...
func init() {
	initLog()
	log.Info("token is:" + os.Getenv("ACCESSCOOKIE"))
	register()
	getCookie()
}
